...
Window_Help.prototype.initialize = function (rect) {
    rect.y = Graphics.boxHeight - this.fittingHeight(1); // Position at the bottom of the screen
    rect.height = this.fittingHeight(1); // Set height for 1 line of text
    Window_Base.prototype.initialize.call(this, rect);
    this._text = "";
};
...
Window_MenuCommand.prototype.addOriginalCommands = function() {
    ...
    this.addCommand("Journal", "journal", true);
    this.addCommand("Achievements", "achievements", true);
    this.addCommand("Encyclopedia", "encyclopedia", true);
};
...
Window_ItemList.prototype.initialize = function (rect) {
    rect.height = this.lineHeight() * 14.8;
    Window_Selectable.prototype.initialize.call(this, rect);
    this._category = "none";
    this._data = [];
};
...
Window_SkillType.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 5.6; // Start after the status window based on line height
    rect.height = this.lineHeight() * 12.46; 
    Window_Command.prototype.initialize.call(this, rect);
    this._actor = null;
};
...
Window_SkillStatus.prototype.initialize = function (rect) {
    // Adjust the rectangle to cover the full horizontal length
    rect.width = Graphics.boxWidth; // Set the width to the full width of the screen
    rect.height = this.lineHeight() * 4.2; 
    Window_StatusBase.prototype.initialize.call(this, rect);
    this._actor = null;
};
...
Window_SkillList.prototype.initialize = function (rect) {
    rect.x = 0; // Start at the left side of the screen
    rect.y = this.lineHeight() * 5.6; // Start after the status window based on line height
    rect.width = Graphics.boxWidth * .7;

    const helpWindowHeight = this.lineHeight() * 1.715; // Assuming the help window is 2 lines tall (adjust as needed)
    rect.height = Graphics.boxHeight - rect.y - helpWindowHeight; // Calculate height to avoid overlap with help window
...
Window_EquipStatus.prototype.initialize = function (rect) {
    rect.height = this.lineHeight() * 16.7; // Adjust height to fit more lines with reduced spacing
    Window_StatusBase.prototype.initialize.call(this, rect);
    this._actor = null;
...
Window_EquipStatus.prototype.refresh = function () {
    ...
    if (this._actor) {
        ...
        const startY = this.lineHeight() * 5; // Adjusted to start parameters lower to avoid overlapping
        this.drawActorName(this._actor, nameRect.x, 0, nameRect.width);
        this.drawActorFace(this._actor, nameRect.x, nameRect.height);
        this.drawAllParams(startY); // Passing the starting Y position
    }
};

Window_EquipStatus.prototype.drawAllParams = function (startY) {
    // Draw default parameters: HP, ATK, DEF, MAT, MDF, AGI, LUK (skip MP)
    const paramsToDraw = [0, 2, 3, 4, 5, 6, 7]; // Indices of parameters to draw (0 = HP, 2 = ATK, 3 = DEF, etc.)

    for (let i = 0; i < paramsToDraw.length; i++) {
        const x = this.itemPadding();
        const y = this.paramY(i, startY);
        this.drawBasicParam(x, y, paramsToDraw[i]);
    }

    // Define only the desired extended/special parameters: HIT, CRIT, EVA, MEV, TGR, PHA
    const extendedParamsToDraw = [
        { name: "Accuracy", value: "hit", type: "xparam" },  // Hit Rate
        { name: "Impact", value: "cri", type: "xparam" }, // Critical Rate
        { name: "Dodge", value: "eva", type: "xparam" },  // Evasion
        { name: "Avoidance", value: "mev", type: "xparam" },  // Magic Evasion
        { name: "Provocation", value: "tgr", type: "sparam" },  // Target Rate
        { name: "Potency", value: "pha", type: "sparam" },  // Pharmacology
    ];

    for (let i = 0; i < extendedParamsToDraw.length; i++) {
        const x = this.itemPadding();
        const y = this.paramY(paramsToDraw.length + i, startY); // Start after basic parameters
        this.drawParam(x, y, extendedParamsToDraw[i]);
    }
};


Window_EquipStatus.prototype.drawBasicParam = function (x, y, paramId) {
    ...
    if (this._actor) {
        const currentValue = this._actor.param(paramId);
        this.drawText(currentValue, paramX, y, paramWidth, "right");
    }
    ...
    if (this._tempActor) {
        ...
        // Set text color based on positive or negative value
        if (diffValue > 0) {
            this.changeTextColor("#00e060"); // Green color for positive values
        } else if (diffValue < 0) {
            this.changeTextColor("#ff784c"); // Red color for negative values
        } else {
            this.resetTextColor(); // Neutral if no change
        }

        this.drawText(newValue, paramX + paramWidth + rightArrowWidth, y, paramWidth, "right");
    }
};

Window_EquipStatus.prototype.drawParam = function (x, y, param) {
    const paramX = this.paramX();
    const paramWidth = this.paramWidth();
    const rightArrowWidth = this.rightArrowWidth();
    ...
    if (this._actor) {
        const currentValue = this.getParamValue(this._actor, param);
        this.drawText(currentValue, paramX, y, paramWidth, "right");
    }
    ...
    if (this._tempActor) {
        const newValue = this.getParamValue(this._tempActor, param);
        const currentValue = this.getParamValue(this._actor, param);
        const diffValue = parseFloat(newValue) - parseFloat(currentValue);
...
Window_EquipStatus.prototype.getParamValue = function (actor, param) {
    switch (param.type) {
        case "xparam":
            return (actor[param.value] * 100).toFixed(2) + "%";
        case "sparam":
            return (actor[param.value] * 100).toFixed(2) + "%";
        default:
            return 0;
...
Window_EquipStatus.prototype.rightArrowWidth = function () {
    return 24; // Reduced width for better fit
};
...
// Adjusted to independently reduce vertical spacing and maintain the starting offset
Window_EquipStatus.prototype.paramY = function (index, startY) {
    const spacing = this.lineHeight() * .85; // Adjust this value to reduce or increase the spacing between each parameter
    return startY + spacing * index; // The parameter start position plus the spacing for each subsequent parameter
};
...
Window_EquipSlot.prototype.initialize = function (rect) {
    rect.height = this.lineHeight() * 14.78;
    Window_StatusBase.prototype.initialize.call(this, rect);
    this._actor = null;
    this.refresh();
};
...
Window_EquipSlot.prototype.drawItem = function (index) {
    if (this._actor) {
        ...
        // Adding a custom vertical offset to control the starting point of each line
        const customOffsetY = 5; // Adjust this value to control the text position vertically within the item area

        this.changeTextColor(ColorManager.systemColor());
        this.changePaintOpacity(this.isEnabled(index));
        this.drawText(slotName, rect.x, rect.y + customOffsetY, slotNameWidth, rect.height);
        this.drawItemName(item, rect.x + slotNameWidth, rect.y + customOffsetY, itemWidth);
        this.changePaintOpacity(true);
    }
};
...
// Add a method to calculate the height of each item
Window_EquipSlot.prototype.itemHeight = function () {
    return this.lineHeight() * 1.28; // Adjust this value to change the height of each item
};

// Modify itemLineRect to adjust the item position and spacing
Window_EquipSlot.prototype.itemLineRect = function (index) {
    const rect = new Rectangle();
    rect.x = this.itemPadding();
    rect.y = index * this.itemHeight(); // Use itemHeight() to calculate the y position for better control
    rect.width = this.innerWidth - this.itemPadding() * 2;
    rect.height = this.itemHeight(); // Set the height of each item
    return rect;
};
...
Window_EquipItem.prototype.initialize = function (rect) {
    rect.height = this.lineHeight() * 14.9;
...
Window_Status.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 1.5;
    rect.height = this.lineHeight() * 5;
    Window_StatusBase.prototype.initialize.call(this, rect);
    this._actor = null;
    this.refresh();
    this.activate();
...
Window_Status.prototype.block2Y = function() {
    ...
    const max = this.innerHeight - lineHeight * 1;
    return Math.floor(lineHeight * .8);
};
...
Window_Status.prototype.drawLevelGauge = function (x, y, width, rate, color1, color2) {
    const fillW = Math.floor(width * rate);
    const gaugeHeight = this.lineHeight() / 2;  // Adjust the height of the gauge as needed
    this.contents.fillRect(x, y, width, gaugeHeight, color2);  // Draw background
    this.contents.fillRect(x, y, fillW, gaugeHeight, color1);  // Draw filled part
};
...
Window_Status.prototype.drawExpInfo = function(x, y) {
    ...
    // Calculate experience rate for the gauge
    const currentExp = this._actor.currentExp();
    const nextLevelExp = this._actor.nextRequiredExp();
    const rate = currentExp / nextLevelExp;

    // Draw level gauge
    const gaugeX = x + 0; // Horizontal position of the gauge
    const gaugeY = y + lineHeight * 1.2; // Vertical position of the gauge
    const gaugeWidth = 180; // Width of the level gauge
    const color1 = ColorManager.systemColor(); // Foreground color
    const color2 = ColorManager.gaugeBackColor(); // Background color
    ...
    this.drawLevelGauge(gaugeX, gaugeY, gaugeWidth, rate, color1, color2);
...
Window_StatusParams.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 6.4; // Adjust vertical placement
    rect.height = this.lineHeight() * 13.3; // Set total height
    Window_StatusBase.prototype.initialize.call(this, rect);
    this._actor = null;
};
...
Window_StatusParams.prototype.maxItems = function () {
    return 12; // Always include all 12 parameters
};

Window_StatusParams.prototype.itemHeight = function () {
    return this.height / this.maxItems() * .945; // Equal height for all items
};

Window_StatusParams.prototype.drawItem = function (index) {
    const itemHeight = this.itemHeight(); // Height for each item
    const rectY = index * itemHeight; // Vertical position for each item
    const rect = new Rectangle(20, rectY, this.contents.width - 40, itemHeight); // Add horizontal margin (20px on each side)

    // Draw default parameters (ATK, DEF, MAT, MDF, AGI, LUK)
    if (index < 6) {
        const paramId = index + 2; // Default parameters start from paramId 2 (ATK)
        const name = TextManager.param(paramId);
        const value = this._actor.param(paramId);
        this.changeTextColor(ColorManager.systemColor());
        this.drawText(name, rect.x, rect.y, 120); // Align parameter names
        this.resetTextColor();
        this.drawText(value, rect.x + 150, rect.y, 60, "right"); // Align parameter values
    }
    // Draw extra parameters (HIT, CRIT, EVA, MEV, TGR, PHA)
    else {
        const extraParams = [
            { name: "Accuracy", value: (this._actor.hit * 100).toFixed(2) + "%" },
            { name: "Impact", value: (this._actor.cri * 100).toFixed(2) + "%" },
            { name: "Dodge", value: (this._actor.eva * 100).toFixed(2) + "%" },
            { name: "Avoidance", value: (this._actor.mev * 100).toFixed(2) + "%" },
            { name: "Provocation", value: (this._actor.tgr * 100).toFixed(2) + "%" },
            { name: "Potency", value: (this._actor.pha * 100).toFixed(2) + "%" },
        ];
        const extraIndex = index - 6; // Adjust index for extra parameters
        const param = extraParams[extraIndex];
        this.changeTextColor(ColorManager.systemColor());
        this.drawText(param.name, rect.x, rect.y, 120); // Align parameter names
        this.resetTextColor();
        this.drawText(param.value, rect.x + 150, rect.y, 60, "right"); // Align parameter values
    }
};

Window_StatusParams.prototype.drawItemBackground = function (/*index*/) {
    // Do nothing to prevent background rendering
};

// Disable scrolling behavior
Window_StatusParams.prototype.isOverflow = function () {
    return false; // Prevent the window from thinking it has overflow content
};

// Ensure contentsHeight matches visible area
Window_StatusParams.prototype.contentsHeight = function () {
    return this.height; // Match content height to window height
};
...
Window_StatusEquip.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 6.4;
    rect.height = this.lineHeight() * 13.3;
    Window_StatusBase.prototype.initialize.call(this, rect);
    this._actor = null;
};
...
Window_ShopBuy.prototype.initialize = function (rect) {
    rect.height = this.lineHeight() * 14.805;
    Window_Selectable.prototype.initialize.call(this, rect);
    this._money = 0;
};
...
Window_PartyCommand.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 12.9;
    rect.height = this.lineHeight() * 6.9; 
    Window_Command.prototype.initialize.call(this, rect);
    this.openness = 0;
    this.deactivate();
};
...
Window_EnemyList.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 1.5;
    rect.height = Graphics.boxHeight * .92;
    Window_Selectable.prototype.initialize.call(this, rect);
    this._enemies = this.validEnemies();
    this.refresh();
    this.select(0);
};
...
// Get a list of valid enemies (that have been encountered)
Window_EnemyList.prototype.validEnemies = function () {
    return $dataEnemies.filter(enemy => enemy && enemy.name && enemy.meta.bestiary !== "false");
};
...
Window_EnemyList.prototype.drawItem = function (index) {
    ...
    if (enemy) {
        const rect = this.itemRect(index);
        const padding = 10; // Adjust this value as needed to move the text to the right
        let name = "?????"; // Default to "?????" for unknown enemies

        if ($gameSystem.isEnemyKnown(enemy.id)) {
            name = enemy.name; // Use the actual name if the enemy is known
        }

        // Draw text with padding applied to move it to the right
        this.drawText(name, rect.x + padding, rect.y, rect.width - padding, 'left');
    }
};
...
Window_EnemyList.prototype.selectedEnemy = function () {
    return this._enemies[this.index()];
};

// Set the details window to update when an enemy is selected
Window_EnemyList.prototype.setDetailsWindow = function (detailsWindow) {
    this._detailsWindow = detailsWindow;
    this.callUpdateDetails();
};

Window_EnemyList.prototype.callUpdateDetails = function () {
...
Window_EnemyList.prototype.update = function () {
    Window_Selectable.prototype.update.call(this);
    this.callUpdateDetails();
};

// Define the Enemy Details Window
function Window_EnemyDetails() {
    this.initialize(...arguments);
}

Window_EnemyDetails.prototype = Object.create(Window_Base.prototype);
Window_EnemyDetails.prototype.constructor = Window_EnemyDetails;

Window_EnemyDetails.prototype.initialize = function (rect) {
    rect.y = this.lineHeight() * 1.5;
    rect.height = Graphics.boxHeight * .92;
    Window_Base.prototype.initialize.call(this, rect);
    this._enemy = null;
...
// Set the enemy to show in the details window
Window_EnemyDetails.prototype.setEnemy = function (enemy) {
...
Window_EnemyDetails.prototype.refresh = function () {
    this.contents.clear();
    if (this._enemy) {
        const lineHeight = this.lineHeight();

        // Determine if the enemy is known
        const isKnown = $gameSystem.isEnemyKnown(this._enemy.id);

        // Display Enemy Image (if available)
        if (isKnown && this._enemy.battlerName) {
            ...
            const imageX = (this.contentsWidth() - newWidth) / 2;
            const imageY = this.lineHeight() * 0.5;

            this.contents.blt(bitmap, 0, 0, bitmap.width, bitmap.height, imageX, imageY, newWidth, newHeight);
        }

        const startY = lineHeight * 4;

        const nameStartY = startY - lineHeight * .4;
        const name = isKnown ? this._enemy.name : "?????";
        this.drawText(name, 0, nameStartY, this.contentsWidth(), 'center');

        const hpMpTpStartY = startY + lineHeight * 0.6;
        this.drawHpMpTp(hpMpTpStartY, isKnown);

        const statsStartY = hpMpTpStartY + lineHeight * 2;
        const stats = [
            { label: "ATK", value: isKnown ? this._enemy.params[2] : "?????" },
            { label: "DEF", value: isKnown ? this._enemy.params[3] : "?????" },
            { label: "AGI", value: isKnown ? this._enemy.params[6] : "?????" },
            { label: "MAT", value: isKnown ? this._enemy.params[4] : "?????" },
            { label: "MDF", value: isKnown ? this._enemy.params[5] : "?????" },
            { label: "LUK", value: isKnown ? this._enemy.params[7] : "?????" },
        ];

        const colWidth = this.contentsWidth() / 2;
        for (let i = 0; i < stats.length; i++) {
            const stat = stats[i];
            const x = (this.contentsWidth() - colWidth * 2) / 2 + (i < stats.length / 2 ? 0 : colWidth);
            const y = statsStartY + lineHeight * 0.8 * Math.floor(i % (stats.length / 2));
            this.drawText(`${stat.label}: ${stat.value}`, x, y, colWidth - 10, 'center');
        }

        const spStatsStartY = statsStartY + lineHeight * 3;
        this.drawSpStats(spStatsStartY, isKnown);

        const exStatsStartY = spStatsStartY + lineHeight * 3;
        this.drawExStats(exStatsStartY, isKnown);

        // Display Stolen Items from the global data structure
        const enemyLoot = $gameSystem._enemyLootData ? $gameSystem._enemyLootData[this._enemy.id] : null;
        const stolenItemsHeader = "Stolen Items:";
        this.drawText(stolenItemsHeader, 0, exStatsStartY + lineHeight * 1.4, this.contentsWidth(), 'center');
        const stolenItemsText = isKnown ?
            (enemyLoot && enemyLoot.length > 0 ? enemyLoot.map(loot => `${loot.name}`).join(", ") : "None")
            : "?????";
        this.drawText(stolenItemsText, 0, exStatsStartY + lineHeight * 2.1, this.contentsWidth(), 'center');

        // Display Dropped Items
        const dropItemsHeader = "Dropped Items:";
        this.drawText(dropItemsHeader, 0, exStatsStartY + lineHeight * 3.2, this.contentsWidth(), 'center');
        let dropItemsText = isKnown ? "None" : "?????";

        if (isKnown && this._enemy.dropItems && this._enemy.dropItems.length > 0) {
            const droppedItems = this._enemy.dropItems.filter(drop => {
                return drop.kind > 0 && (
                    (drop.kind === 1 && $dataItems[drop.dataId]) ||
                    (drop.kind === 2 && $dataWeapons[drop.dataId]) ||
                    (drop.kind === 3 && $dataArmors[drop.dataId])
                );
            }).map(drop => {
                let itemName = "Unknown Item";
                switch (drop.kind) {
                    case 1: // Item
                        itemName = $dataItems[drop.dataId]?.name || "Unknown Item";
                        break;
                    case 2: // Weapon
                        itemName = $dataWeapons[drop.dataId]?.name || "Unknown Weapon";
                        break;
                    case 3: // Armor
                        itemName = $dataArmors[drop.dataId]?.name || "Unknown Armor";
                        break;
                }
                return itemName;
            });
            ...
            if (droppedItems.length > 0) {
                dropItemsText = droppedItems.join(", ");
            }
        }

        this.drawText(dropItemsText, 0, exStatsStartY + lineHeight * 3.9, this.contentsWidth(), 'center');
    }
};

Window_EnemyDetails.prototype.drawHpMpTp = function (startY, isKnown) {
    const lineHeight = this.lineHeight();
    const hpMpTpStats = [
        { label: "HP", value: isKnown ? this._enemy.params[0] : "?????" },
        { label: "MP", value: isKnown ? this._enemy.params[1] : "?????" },
        { label: "TP", value: isKnown ? this._enemy.params[2] : "?????" },
    ];

    const colWidth = this.contentsWidth();
    this.drawText(`${hpMpTpStats[0].label}: ${hpMpTpStats[0].value}`, 0, startY, colWidth, 'center');
    const mpTpY = startY + lineHeight;
    this.drawText(`${hpMpTpStats[1].label}: ${hpMpTpStats[1].value}   ${hpMpTpStats[2].label}: ${hpMpTpStats[2].value}`, 0, mpTpY, colWidth, 'center');
};

Window_EnemyDetails.prototype.drawSpStats = function (startY, isKnown) {
    const lineHeight = this.lineHeight();

    // Ensure this._enemy is defined and contains the stats
    if (!this._enemy) {
        console.error("Enemy data is not defined");
        return;
    }
    ...
    // Extract stats from the traits array if the enemy is known
    if (isKnown) {
        this._enemy.traits.forEach(trait => {
            if (trait.code === 22) { // Code 22 represents parameter traits
                switch (trait.dataId) {
                    case 0: // HIT
                        hit = `${(trait.value * 100).toFixed(2)}%`;
                        break;
                    case 1: // EVA
                        evaRate = `${(trait.value * 100).toFixed(2)}%`;
                        break;
                    case 2: // CRIT
                        critRate = `${(trait.value * 100).toFixed(2)}%`;
                        break;
                    case 4: // MEV
                        magicEvaRate = `${(trait.value * 100).toFixed(2)}%`;
                        break;
                }
            }
        });
    ...
    console.log("HIT:", hit);
    console.log("CRIT:", critRate);
    console.log("EVA:", evaRate);
    console.log("MEV:", magicEvaRate);

    const spstats = [
        { label: "HIT", value: hit,},
        { label: "CRIT", value: critRate },
        { label: "EVA", value: evaRate },
        { label: "MEV", value: magicEvaRate },
    ];

    // Draw the spstats in two columns
    const colWidth = this.contentsWidth() / 2;
    for (let i = 0; i < spstats.length; i++) {
        const stat = spstats[i];
        const x = (this.contentsWidth() - colWidth * 2) / 2 + (i < spstats.length / 2 ? 0 : colWidth);
        const y = startY + lineHeight * Math.floor(i % (spstats.length / 2));
        this.drawText(`${stat.label}: ${stat.value}`, x, y, colWidth - 10, 'center');
    }
};

// Function to draw extra stats (EXP, GOLD)
Window_EnemyDetails.prototype.drawExStats = function (startY, isKnown) {
    const lineHeight = this.lineHeight();
    const exstats = [
        { label: "EXP", value: isKnown ? this._enemy.exp : "?????" },
        { label: "GOLD", value: isKnown ? this._enemy.gold : "?????" }
    ];

    // Draw each extra stat, aligned horizontally
    const colWidth = this.contentsWidth() / 2;
    for (let i = 0; i < exstats.length; i++) {
        const stat = exstats[i];
        const x = i * colWidth;
        this.drawText(`${stat.label}: ${stat.value}`, x, startY, colWidth - 10, 'center');
    }
};
