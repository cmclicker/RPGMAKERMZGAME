...
// Store the original Game_System initialize function
const _Game_System_initialize = Game_System.prototype.initialize;

// Extend the Game_System initialize function
Game_System.prototype.initialize = function () {
    // Call the original initialize method
    _Game_System_initialize.call(this);

    // Add custom properties after original initialization
    this._knownEnemies = []; // Array to keep track of known enemies
    this.initGlobalLootData();
};



// Mark an enemy as defeated
Game_System.prototype.markEnemyAsDefeated = function (enemyId) {
    if (!this._knownEnemies.includes(enemyId)) {
        this._knownEnemies.push(enemyId);
    }
};

// Check if an enemy is known (i.e., defeated at least once)
Game_System.prototype.isEnemyKnown = function (enemyId) {
    return this._knownEnemies.includes(enemyId);
};

// Create a new property to track loot globally
Game_System.prototype.initGlobalLootData = function () {
    if (!this._enemyLootData) {
        this._enemyLootData = {};
    }
};

// Utility function to save loot globally
Game_System.prototype.addLootForEnemy = function (enemyId, loot) {
    if (!this._enemyLootData[enemyId]) {
        this._enemyLootData[enemyId] = [];
    }
    this._enemyLootData[enemyId].push(loot);
};

// Utility function to get loot for an enemy
Game_System.prototype.getLootForEnemy = function (enemyId) {
    return this._enemyLootData && this._enemyLootData[enemyId] ? this._enemyLootData[enemyId] : [];
};
