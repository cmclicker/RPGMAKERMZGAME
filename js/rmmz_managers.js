...
let _isProcessingVictory = false;

BattleManager.processVictory = function () {
    if (_isProcessingVictory) {
        console.warn("processVictory is already running, returning early to prevent recursion.");
        return; // Prevent re-entry
    }

    _isProcessingVictory = true;

    // Custom logic to mark enemies as defeated
    $gameTroop.members().forEach(enemy => {
        if (enemy.isEnemy() && enemy.isDead()) {
            $gameSystem.markEnemyAsDefeated(enemy.enemyId());
        }
    });

    // Original processVictory logic without recursion
