...
Scene_Menu.prototype.createCommandWindow = function() {
    ...
    commandWindow.setHandler("bestiary", this.commandBestiary.bind(this));
    commandWindow.setHandler("journal", this.commandJournal.bind(this));
    commandWindow.setHandler("achievements", this.commandAchievements.bind(this));
    commandWindow.setHandler("encyclopedia", this.commandEncyclopedia.bind(this));
...
Scene_Bestiary.prototype.create = function () {
    ...
    // Create the Enemy List Window
    const listWindowRect = new Rectangle(0, 0, 300, Graphics.boxHeight);
    this._enemyListWindow = new Window_EnemyList(listWindowRect);
    this._enemyListWindow.setHandler('cancel', this.popScene.bind(this)); // Allow canceling to exit
    this.addWindow(this._enemyListWindow);

    // Create the Enemy Details Window
    const detailsWindowRect = new Rectangle(300, 0, Graphics.boxWidth - 300, Graphics.boxHeight);
    this._enemyDetailsWindow = new Window_EnemyDetails(detailsWindowRect);
    this.addWindow(this._enemyDetailsWindow);

    // Link the selection change in the list to update the details window
    this._enemyListWindow.setDetailsWindow(this._enemyDetailsWindow);

    // Set the Enemy List Window to be active but without input other than scrolling
    this._enemyListWindow.activate();
    this._enemyListWindow.select(0);
};


// Add content for the bestiary in this scene
...
// Add content for the journal in this scene
...
// Add content for the achievements in this scene

function Scene_Encyclopedia() {
    this.initialize(...arguments);
}
...
Scene_Encyclopedia.prototype.initialize = function () {
    Scene_MenuBase.prototype.initialize.call(this);
};

// Add content for the encyclopedia in this scene
...
Scene_Shop.prototype.dummyWindowRect = function() {
    ...
    const ww = Graphics.boxWidth;
    const wh = this.mainAreaHeight() * .945;
    return new Rectangle(wx, wy, ww, wh);
};
